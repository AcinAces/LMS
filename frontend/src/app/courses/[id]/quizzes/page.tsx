      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <div className="bg-blue-900/30 border border-blue-500/30 rounded-xl p-6 mb-12 backdrop-blur-md">
          ...
          <ul className="space-y-2 text-blue-200 list-disc list-inside">
            <li>{t('quiz.rule_internet')}</li>
            <li>{t('quiz.rule_timer')}</li>
            <li>{t('quiz.rule_tabs')}</li>
            <li>{t('quiz.rule_auto_submit')}</li>
            <li>
              {t('quiz.rule_proctor')}{' '}
              <button onClick={() => setShowViolations(true)} className="text-blue-300 underline hover:text-blue-200 cursor-pointer">
                {t('quiz.click_violations')}
              </button>
            </li>
          </ul>
        </div>
              ...
              <div key={quiz.id} className="bg-white/5 border border-white/10 rounded-xl p-6 hover:border-emerald-500/50 transition-all group backdrop-blur-sm flex flex-col">
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  ...
                  <div className="bg-black/40 rounded-lg p-3 text-center border border-white/5">
                    ...
                    <div className="text-xs text-gray-400 uppercase tracking-wider">{t('quiz.marks')}</div>
                  </div>
                  <div className="bg-black/40 rounded-lg p-3 text-center border border-white/5">
                    <div className="text-amber-500 font-bold text-xl">{quiz.timeLimit}</div>
                    <div className="text-xs text-gray-400 uppercase tracking-wider">{t('quiz.minutes')}</div>
                  </div>
                </div>

                <button
                  onClick={() => setSelectedQuiz(quiz)}
                  className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold transition-all shadow-lg shadow-emerald-500/20 cursor-pointer"
                >
                  {t('quiz.start_quiz')}
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-gray-900 border border-white/10 rounded-xl w-full max-w-md shadow-2xl p-6">
            <div className="flex items-center gap-3 mb-4 text-amber-500">
              ...
              <h3 className="text-xl font-bold text-white">{t('quiz.confirm_title')}</h3>
            </div>
            <p className="text-gray-300 mb-8 leading-relaxed">
              {t('quiz.confirm_proctored')}
            </p>
            <div className="flex items-center gap-4">
              <button
                onClick={() => setSelectedQuiz(null)}
                className="flex-1 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg font-bold transition-colors cursor-pointer"
              >
                {t('quiz.cancel')}
              </button>
              <button
                onClick={() => router.push(`/courses/${params.id}/quizzes/${selectedQuiz.documentId}`)}
                className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold transition-colors cursor-pointer"
              >
                {t('quiz.proceed')}
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-gray-900 border border-white/10 rounded-xl w-full max-w-lg shadow-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-white">{t('quiz.violations_title')}</h3>
              <button onClick={() => setShowViolations(false)} className="text-gray-400 hover:text-white cursor-pointer">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
              </button>
            </div>
            <ul className="space-y-4 mb-8">
              <li className="flex items-center justify-between">
                 <span className="text-gray-300">{t('quiz.v_fullscreen')}</span>
                 <span className="text-red-500 font-bold bg-red-500/10 px-3 py-1 rounded-md border border-red-500/20">{t('quiz.v_fullscreen_penalty')}</span>
              </li>
              <li className="flex items-center justify-between">
                 <span className="text-gray-300">{t('quiz.v_minimize')}</span>
                 <span className="text-red-500 font-bold bg-red-500/10 px-3 py-1 rounded-md border border-red-500/20">{t('quiz.v_minimize_penalty')}</span>
              </li>
              <li className="flex items-center justify-between">
                 <span className="text-gray-300">{t('quiz.v_tabs')}</span>
                 <span className="text-red-500 font-bold bg-red-500/10 px-3 py-1 rounded-md border border-red-500/20">{t('quiz.v_tabs_penalty')}</span>
              </li>
            </ul>
            <div className="bg-red-900/20 border border-red-500/30 p-4 rounded-lg mb-8">
              <p className="text-red-400 text-sm font-bold flex items-center gap-2">
                 ...
                 {t('quiz.violations_warning')}
              </p>
            </div>
            <button
              onClick={() => setShowViolations(false)}
              className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-bold transition-colors cursor-pointer"
            >
              {t('quiz.i_understand')}
            </button>
          </div>
