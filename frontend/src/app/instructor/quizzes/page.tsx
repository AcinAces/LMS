  ...
  const fetchQuizzesAndData = async () => {
    try {
      ...
      const [quizzesRes, coursesRes, enrollmentsRes, attemptsRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:1337'}/api/quizzes?populate[course]=true&populate[questions]=true&filters[course][courseAuthor][id][$eq]=${user?.id}&sort=createdAt:desc`, {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400">Create quizzes for your authored courses, monitor submission rates, and track class scores.</p>
      </div>

      <DataTable 
        title="My Course Quizzes"
        columns={columns}
        data={quizzes}
