...
export default function ContentManagerDashboard() {
  ...
  const statCards = [
    { label: 'Active Courses', value: stats.courses, icon: '📚', color: 'from-cyan-500/20 to-blue-500/10', border: 'border-cyan-500/30', text: 'text-cyan-400', href: '/content-manager/courses' },
    { label: 'Published Lessons', value: stats.lessons, icon: '🎬', color: 'from-purple-500/20 to-indigo-500/10', border: 'border-purple-500/30', text: 'text-purple-400', href: '/content-manager/lessons' },
    { label: 'Anti-Cheat Quizzes', value: stats.quizzes, icon: '🛡️', color: 'from-amber-500/20 to-orange-500/10', border: 'border-amber-500/30', text: 'text-amber-400', href: '/content-manager/quizzes' },
    { label: 'Published Blogs', value: stats.blogs, icon: '✍️', color: 'from-rose-500/20 to-pink-500/10', border: 'border-rose-500/30', text: 'text-rose-400', href: '/content-manager/blogs' },
  ];

  const quickActions = [
    { href: '/content-manager/courses', title: 'Manage Courses', desc: 'Create, edit, and organize all curriculum courses', icon: '📚', color: 'text-cyan-400 hover:border-cyan-500/40' },
    { href: '/content-manager/lessons', title: 'Manage Video Lessons', desc: 'Add video lessons, duration times, and reading content', icon: '🎬', color: 'text-purple-400 hover:border-purple-500/40' },
    { href: '/content-manager/quizzes', title: 'Create Quizzes', desc: 'Author interactive anti-cheat examination questions', icon: '🛡️', color: 'text-amber-400 hover:border-amber-500/40' },
    { href: '/content-manager/progress', title: 'Student Progress', desc: 'Review student performance and quiz evaluation metrics', icon: '📊', color: 'text-teal-400 hover:border-teal-500/40' },
    { href: '/content-manager/blogs', title: 'Manage Educational Blogs', desc: 'Write, publish, and edit rich tech articles across all topics', icon: '✍️', color: 'text-indigo-400 hover:border-indigo-500/40' },
  ];
    ...
    <div className="space-y-10 animate-fade-in-up">
      ...
      <div className="bg-gradient-to-r from-slate-900/90 via-slate-900/60 to-slate-950/90 border border-white/10 rounded-3xl p-6 sm:p-8 backdrop-blur-2xl shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative overflow-hidden">
        ...
        <div className="space-y-1 relative z-10">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 font-mono">Curriculum Operations</span>
          </div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-white tracking-tight">
            Content Manager Dashboard
          </h1>
          <p className="text-xs sm:text-sm text-slate-400">
            Create, audit, and publish curriculum courses, quizzes, and educational articles.
          </p>
        </div>
        ...
        <div className="relative z-10 flex items-center gap-2">
          <Link 
            ...
            className="px-4 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-xs shadow-lg shadow-cyan-500/20 transition-all flex items-center gap-1.5"
          >
      ...
      <div className="bg-slate-900/60 backdrop-blur-2xl border border-white/10 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <div>
            <h2 className="text-xl font-bold text-white tracking-tight">Content Studio Actions</h2>
            <p className="text-xs text-slate-400 mt-0.5">Manage and update published curriculum and study material</p>
          </div>
          <span className="text-xs font-mono text-slate-500 hidden sm:inline">5 Actions Available</span>
        </div>
