...
export default function ContentManagerLessonsPage() {
  ...
  const fetchLessonsAndCourses = async () => {
    try {
      ...
      const [lessonsRes, coursesRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:1337'}/api/lessons?populate=course&sort=order:asc&pagination[limit]=1000`, {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400">Add video lessons, configure sequence numbers, and preview content.</p>
      </div>
