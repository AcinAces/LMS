...
export default function AdminCoursesPage() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      const [coursesRes, instructorsRes, enrollmentsRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:1337'}/api/courses?populate[courseAuthor]=true&populate[lessons]=true&populate[quizzes]=true`, {
    ...
    { 
      ...
      render: (row) => (
        <span className="text-emerald-400 font-medium text-xs">
          {row.courseAuthor?.username || 'Unassigned'}
        </span>
      )
    },
    { 
  ...
  const fields: FormField[] = [
    ...
    { 
      ...
      type: 'select', 
      required: true, 
      placeholder: 'Select assigned instructor...',
      options: instructors.map(i => ({ value: i.documentId || i.id, label: i.username })),
      hint: 'The designated instructor responsible for teaching and managing this course'
    },
    { 
      key: 'courseTag', 
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400">Create, edit, and organize learning paths with real-time student enrollment counts.</p>
      </div>
