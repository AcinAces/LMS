...
async function run() {
  ...
  const roleRes = await client.query("SELECT id FROM up_roles WHERE type = 'instructor'");
  if (roleRes.rows.length === 0) throw new Error('Instructor role not found');
  const instructorId = roleRes.rows[0].id;
  ...
  for (const action of actions) {
    ...
    const link = await client.query("SELECT 1 FROM up_permissions_role_lnk WHERE permission_id = $1 AND role_id = $2", [permId, instructorId]);
    if (link.rows.length === 0) {
      await client.query("INSERT INTO up_permissions_role_lnk (permission_id, role_id) VALUES ($1, $2)", [permId, instructorId]);
      console.log('Granted', action);
    }
  }

  // Revoke any unintended blog or user modification permissions just in case
  const removeActions = [
    'api::blog.blog.create', 'api::blog.blog.update', 'api::blog.blog.delete', 'api::blog.blog.find', 'api::blog.blog.findOne',
    'plugin::users-permissions.user.create', 'plugin::users-permissions.user.update', 'plugin::users-permissions.user.destroy'
  ];
  for(const action of removeActions) {
    let perm = await client.query("SELECT id FROM up_permissions WHERE action = $1", [action]);
    if (perm.rows.length > 0) {
        await client.query("DELETE FROM up_permissions_role_lnk WHERE permission_id = $1 AND role_id = $2", [perm.rows[0].id, instructorId]);
    }
  }

  console.log('Instructor permissions configured!');
  await client.end();
}
